...
fetch("data/JSON.json").then((response)=>
		response.json()
		).then((myObject)=>
		{
...
{
	let cardDiv=`<img src="image/visa.jpg"><img src="image/mastercard.jpg"><img src="image/amex.png"><img src="image/discover.png">`
	...
	cardDiv+=`<div style="margin:10px 10px auto auto;float:right;">CVV/CVV2:<input type="password" style="appearance:none;padding-left:15px;width:70px;background-image:url('image/lock.jpeg');background-size:15px auto;background-position:1px 5px;background-repeat:no-repeat;" maxlength="3"></div></div>`
